...
<body>
    ...
    <!-- Optional JavaScript -->
      ...
      <script>
	  	$(function(){
	  		//获取所有信息
		  	$.getJSON("${pageContext.request.contextPath}/UserServlet" , {method:"doUserList"} , function(data){
			  		var list = data.list;
		  			var htmlText='';
		  			for(var i= 0 ; i<list.length;i++){
		  				htmlText+='<tr>';
		  				htmlText+='<td>'+list[i].id+'</td>';
		  				htmlText+='<td>'+list[i].username+'</td>';
		  				htmlText+='<td>'+list[i].password+'</td>';
	  	...
	  	function doModify(id){
	  		window.location.href="userModify.jsp?id="+id;
	  	}
	  	function doDelete(id){
	  		if(confirm("请确认是否删除该信息?")){
	  			$.ajax({
	  	            ...
	  	            data: {id:id,method:"doUserDelete"},
	  	            success: function(data){
	  	            	if(data=="true"){
	  	            		alert("用户信息删除成功!");
	  	            		window.location.href="userModify.jsp";
	  	            		return;
	  	            	}
	  	        		alert("用户信息删除失败,请稍后重试!");
	  	            }
	  	        });	
	  		}
	  	}
