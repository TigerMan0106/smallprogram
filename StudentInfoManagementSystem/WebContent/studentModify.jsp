<%@page import="com.hms.model.Student"%>
	    ...
	    $(function(){
	    	...
	    	$.getJSON("${pageContext.request.contextPath}/MajorServlet" , {method:"doMajorList"} , function(data){
			    	...
			    	$.getJSON("${pageContext.request.contextPath}/ClassesServlet" , {method:"doClassesList"} , function(data){
							...
							if(id!=null){
								//根据id获取类型详细信息
								$.getJSON("${pageContext.request.contextPath}/StudentServlet" , {method:"doStudentInfo",id:id} , function(data){
									$("#id").val(data.student.id);
									$("#sid").val(data.student.sid);
									$("#name").val(data.student.name);
									$("#sex").val(data.student.sex);
									$("#idcard").val(data.student.idcard);
									$("#nplace").val(data.student.nplace);
									$("#major").val(data.student.major);
									$("#classes").val(data.student.classes);
	  		...
	  		$("#btn_submit").bind("click",function(){
	    		...
	    		var phone=$("#phone").val();
	    		//检查填写信息
	    		if(id.length==0 || sid.length==0 || name.length==0 || sex.length==0 || idcard.length==0 || nplace.length==0 || major.length==0 || classes.length==0 || phone.length==0){
	    			alert("您还没有补全信息喔!");
	    		}else{
	    			//开始提交后台处理修改信息
	        		$.ajax({
	                    ...
	                    data: {method:"doStudentModify",id:id,sid:sid,name:name,sex:sex,idcard:idcard,nplace:nplace,major:major,classes:classes,phone:phone},
